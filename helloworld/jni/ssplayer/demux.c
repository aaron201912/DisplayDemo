...
extern int g_forward_flag, g_backward_flag;
...
//	AVFormatContext *p_fmt_ctx = is->p_fmt_ctx;
    ...
    while (1)
    {
        ...
        if (is->seek_req) {
            ...
            ret = avformat_seek_file(is->p_fmt_ctx, is->video_idx, seek_min, seek_target, seek_max, is->seek_flags);
            if (ret < 0) {
                ...
                */
                //printf("\033[32;2mseek file successful!\033[0m\n");
            }
            is->seek_req = 0;
             ...
             stream_has_enough_packets(is->p_video_stream, is->video_idx, &is->video_pkt_queue)))
        {
            ...
            outtime.tv_sec = now.tv_sec;
            outtime.tv_nsec = now.tv_usec * 1000 + 100 * 1000 * 1000;//timeout 10ms
...
int exit_demux(player_stat_t *is)
{
    if (is)
    {
        is->abort_request = 1;
